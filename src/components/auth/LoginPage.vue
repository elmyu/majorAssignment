...
// ---------- 面板切换 ----------
function openRegister() { mode.value = 'register'; regError.value = ''; regSuccess.value = '' }
function openForgot() { mode.value = 'forgot'; forgotError.value = ''; forgotSuccess.value = '' }
function backToLogin() { mode.value = 'login'; errorMsg.value = ''; successMsg.value = '' }
      ...
      <div class="form-box">
        ...
        <template v-else-if="mode === 'register'">
          ...
          <label class="field">
            ...
            <input v-model="regForm.phone" type="tel" maxlength="11" placeholder="请输入 11 位手机号" />
          </label>
          <label class="field">
        ...
        <template v-else>
          ...
          <label class="field">
            <span>手机号 <em class="req">*</em></span>
            <input v-model="forgotForm.phone" type="tel" maxlength="11" placeholder="请输入注册时填写的手机号" />
          </label>
          <label class="field">
...
.login-card {
  display: flex; max-width: 840px; width: 100%; overflow: hidden;
  background: #fff; border-radius: 12px;
  box-shadow: 0 16px 40px rgba(45, 90, 50, 0.16); border: 1px solid #e6f0e6;
}
.login-brand {
...
.login-brand::before {
  content: ''; position: absolute; left: -40px; bottom: -60px; width: 170px; height: 170px;
  background: rgba(255, 255, 255, 0.06); border-radius: 50%;
}
.login-brand h1 { font-size: 1.5rem; font-weight: 700; margin: 0 0 8px; letter-spacing: 0.02em; position: relative; z-index: 1; }
.login-brand .brand-sub { margin: 0; font-size: 0.86rem; opacity: 0.9; position: relative; z-index: 1; }

.form-box { flex: 1; display: flex; flex-direction: column; justify-content: center; padding: 36px 40px; }
.form-box h2 { margin: 0 0 16px; font-size: 1.3rem; color: #2c3e2f; }
.form-box .desc { margin: 0 0 14px; font-size: 0.82rem; color: #6b7280; line-height: 1.6; }
.role-select { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-bottom: 16px; }
.role-select.small { grid-template-columns: repeat(2, 1fr); }
.role-item {
  padding: 10px 0; border: 1px solid #dde7dd; border-radius: 8px; cursor: pointer;
  font-size: 0.95rem; font-weight: 600; color: #5a6d5d; background: #fbfdfb; font-family: inherit;
  text-align: center; transition: all 0.15s;
}
.role-item:hover { border-color: #9cc39f; background: #f0f8f0; }
.role-item.active {
  background: linear-gradient(135deg, #4a9d50, #35833c); color: #fff; border-color: #35833c;
  box-shadow: 0 3px 10px rgba(46, 110, 52, 0.25);
}
.row2 { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
.field { display: flex; flex-direction: column; gap: 5px; margin-bottom: 12px; }
.field span { font-size: 0.84rem; font-weight: 600; color: #3f5242; }
.req { color: #c0392b; font-style: normal; }
.field input, .field select {
  padding: 10px 12px; border: 1px solid #dde7dd; border-radius: 8px; font-size: 0.88rem; outline: none; font-family: inherit;
  background: #fbfdfb; transition: border-color 0.15s, background 0.15s;
}
.field input:focus, .field select:focus { border-color: #4a9d50; background: #fff; box-shadow: 0 0 0 3px rgba(74, 157, 80, 0.12); }
.error { color: #c0392b; font-size: 0.82rem; margin: 0 0 10px; }
.success { color: #2e8b57; font-size: 0.82rem; margin: 0 0 10px; }
.login-btn {
  padding: 12px; border: none; border-radius: 8px; color: #fff; font-size: 1rem; font-weight: 700; cursor: pointer;
  background: linear-gradient(90deg, #4a9d50, #35833c); font-family: inherit;
  box-shadow: 0 4px 12px rgba(46, 110, 52, 0.28);
}
.login-btn:hover { background: linear-gradient(90deg, #3d8c43, #2c6e33); }
.auth-links { display: flex; justify-content: space-between; margin-top: 14px; }
.link { font-size: 0.84rem; color: #3d7d5d; cursor: pointer; text-decoration: none; }
.link:hover { color: #2c6e33; text-decoration: underline; }
.quick-fill { font-size: 0.78rem; color: #93a293; margin-top: 14px; }
@media (max-width: 720px) { .login-card { flex-direction: column; } .login-brand { padding: 28px; } .row2 { grid-template-columns: 1fr; } }
</style>
