...
const displayed = computed(() => {
  return users.value.filter(
    (u) => !search.value || u.name.includes(search.value) || u.account.includes(search.value)
  );
});
...
const toggleSelect = (id) => {
  if (users.value.find((u) => u.id === id && u.role === 'admin')) return;
  const s = new Set(selectedIds.value);
...
const isAllSelected = computed(
  () =>
    displayedDeletable.value.length > 0 &&
    displayedDeletable.value.every((d) => selectedIds.value.has(d.id))
);
const toggleSelectAll = () => {
  if (isAllSelected.value) selectedIds.value = new Set();
  else {
    const s = new Set(selectedIds.value);
    displayedDeletable.value.forEach((d) => s.add(d.id));
    selectedIds.value = s;
  }
};
  ...
  try {
    if (editing.value) {
      ...
      if (form.value.role === 'doctor') {
        patch.dept = form.value.dept;
        patch.title = form.value.title;
      } else if (form.value.role === 'patient') {
        patch.gender = form.value.gender;
        patch.age = Number(form.value.age);
...
<template>
  <div class="user-page">
    <div class="head-desc">
      管理员权限：对医生 / 患者账户进行新增、修改、删除、查询；可按住左键拖动框选多行删除。
    </div>
    ...
    <section class="toolbar">
      ...
      <div class="actions">
        ...
        <button class="batch-btn" @click="confirmBatchDel = true" :disabled="batchCount === 0">
          删除选中<span v-if="batchCount" class="cnt">{{ batchCount }}</span>
        </button>
    ...
    <section class="table-card" ref="tableWrap" @mousedown="startDrag">
      ...
      <template v-else>
        ...
        <table v-if="displayed.length" class="user-table">
          ...
          <tbody>
            <tr
              v-for="u in displayed"
              :key="u.id"
              :data-id="u.id"
              class="draggable"
              :class="[selectedIds.has(u.id) ? 'sel' : '', u.role === 'admin' ? 'no-del' : '']"
            >
              <td class="col-check">
                <input
                  type="checkbox"
              ...
              <td>
                <span class="role-badge" :class="'role-' + u.role">{{ ROLE_LABELS[u.role] }}</span>
              </td>
              ...
              <td>
                <div class="ops">
                  <button class="op edit" @click="openEdit(u)">修改</button
                  ><button v-if="u.role !== 'admin'" class="op del" @click="askDelete(u)">
                    删除
                  </button>
                </div>
              </td>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="close">
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="row2">
            <label class="f"
              >角色<select v-model="form.role" :disabled="!!editing">
                <option value="patient">患者</option>
                <option value="doctor">医生</option>
              </select></label
            >
          ...
          <template v-else>
            <div class="row2">
              ...
              >
              <label class="f"
                >年龄<input v-model.number="form.age" type="number" min="0" max="150"
              /></label>
            </div>
            <div class="row2">
              <label class="f"
                >血型<select v-model="form.bloodType">
        ...
        <div class="modal-foot">
          ...
          ><button class="btn save" @click="submit">{{ editing ? '保存修改' : '确认新增' }}</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="confirmDel" class="del-overlay" @click.self="confirmDel = null">
      <div class="del-dialog">
        ...
        <p class="del-text">
          确定要删除用户 <b class="del-name">{{ confirmDel.name }}</b
          ><br />（账号：{{ confirmDel.account }}）<br /><span class="del-warn"
            >此操作不可恢复！</span
          >
        </p>
        <div class="del-btns">
          <button class="del-cancel" @click="confirmDel = null">取消</button
          ><button class="del-confirm" @click="doDelete">确认删除</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="confirmBatchDel" class="del-overlay" @click.self="confirmBatchDel = false">
      <div class="del-dialog">
        ...
        <p class="del-text">
          确定要批量删除选中的 <b class="del-name">{{ batchCount }}</b> 个用户吗？<br /><span
            class="del-warn"
            >此操作不可恢复！</span
          >
        </p>
        <div class="del-btns">
          <button class="del-cancel" @click="confirmBatchDel = false">取消</button
...
.user-table tbody tr.draggable {
  cursor: default;
}
.user-table tbody tr.sel {
  background: #eef6ee;
}
.user-table tbody tr.no-del {
  opacity: 0.6;
}
.col-check {
  width: 40px;
