...
function openAdd() {
  editing.value = null
  form.value = { role: 'patient', account: '', password: '', name: '', dept: '心内科', title: '主治医师', gender: '男', age: 30, bloodType: 'O', phone: '', medicalNo: '' }
  showModal.value = true
}
function openEdit(u) {
  editing.value = u
  form.value = {
    role: u.role, account: u.account, password: '', name: u.name,
    dept: u.dept || '心内科', title: u.title || '主治医师',
    gender: u.gender || '男', age: u.age || 30, bloodType: u.bloodType || 'O',
    phone: u.phone || '', medicalNo: u.medicalNo || '',
  }
  showModal.value = true
}
...
function submit() {
  ...
  try {
    if (editing.value) {
      ...
      createUser({ ...form.value, age: Number(form.value.age), password: form.value.password.trim() })
      showToast('新用户创建成功')
    }
    close()
    load()
...
<template>
  <div class="user-page">
    <div class="head-desc">管理员权限：可对医生 / 患者账户进行新增、删除、修改、查询操作。</div>
    ...
    <section class="table-card">
      ...
      <table v-else-if="displayed.length" class="user-table">
        <thead>
          <tr><th>账号</th><th>姓名</th><th>角色</th><th>科室/性别</th><th>职称/年龄</th><th>电话</th><th>操作</th></tr>
        </thead>
        <tbody>
          <tr v-for="u in displayed" :key="u.id">
            ...
            <td>
              <div class="ops">
                ...
                <button v-if="u.role !== 'admin'" class="op del" @click="askDelete(u)">删除</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="close">
      <div class="modal">
        ...
        <div class="modal-body">
          <div class="row2">
            <label class="f">角色
              <select v-model="form.role" :disabled="!!editing">
                <option value="patient">患者</option>
                <option value="doctor">医生</option>
              </select>
            </label>
            <label class="f">账号 *
              <input v-model="form.account" :disabled="!!editing" />
            </label>
          </div>
          <div class="row2">
            <label class="f">姓名 *
          ...
          <template v-else>
            <div class="row2">
              <label class="f">性别
                <select v-model="form.gender"><option value="男">男</option><option value="女">女</option></select>
              </label>
              <label class="f">年龄
            ...
            <div class="row2">
              <label class="f">血型
                <select v-model="form.bloodType"><option value="A">A</option><option value="B">B</option><option value="O">O</option><option value="AB">AB</option></select>
              </label>
              <label class="f">门诊号
                <input v-model="form.medicalNo" />
              </label>
            </div>
          </template>
          <label class="f">电话
    ...
    <div v-if="confirmDel" class="del-overlay" @click.self="confirmDel = null">
      <div class="del-dialog">
        ...
        <p class="del-text">
          确定要删除用户 <b class="del-name">{{ confirmDel.name }}</b><br />
          （账号：{{ confirmDel.account }}）<br />
...
.filters select { padding: 8px 10px; border: 1px solid #e0e7e0; font-family: inherit; }
