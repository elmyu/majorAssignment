...
const filteredDevices = computed(() => {
  ...
  if (q)
    r = r.filter((d) =>
      [d.name, d.model, d.department, d.code, d.note].some((x) =>
...
const stClass = (s) =>
  ({ 正常使用: 'active', 维修中: 'repair', 闲置: 'idle', 已报废: 'scrapped' })[s] || 'idle';
const runClass = (r) =>
  ({ 在线: 'online', 运行中: 'running', 故障: 'fail', 校准中: 'calib', 离线: 'offline' })[r] ||
  'offline';
...
function onDragMove(e) {
  ...
  dragRect.value = {
    ...
    w: Math.abs(e.clientX - dragStart.value.x),
    h: Math.abs(e.clientY - dragStart.value.y),
  };
  ...
  rows.forEach((row) => {
    ...
    if (hit) s.add(row.dataset.id);
    else s.delete(row.dataset.id);
    selectedIds.value = s;
  });
}
function onDragUp() {
...
const isAllSelected = computed(
  () =>
    ...
    filteredDevices.value.every((d) => selectedIds.value.has(d.id))
);
const toggleSelectAll = () => {
  if (isAllSelected.value) selectedIds.value = new Set();
  else {
    const s = new Set(selectedIds.value);
    filteredDevices.value.forEach((d) => s.add(d.id));
    selectedIds.value = s;
  }
};
...
function submitForm() {
  ...
  if (form.price === null || form.price === '' || form.price < 0)
    return showToast('请输入有效的价格', 'warning');
  const data = {
    name: form.name.trim(),
...
function submitBatch() {
  ...
  showToast(
    invalid ? `成功录入 ${ok} 台（跳过 ${invalid} 行）` : `成功批量录入 ${ok} 台设备`,
    'success'
  );
  closeModal();
  loadDevices();
...
<template>
  <div class="device-manage">
    <div class="head-desc">
      设备物资维护：新增 / 批量录入设备，修改状态；可按住左键拖动框选多行删除。
    </div>

    <section class="stats-bar">
      <button class="chip" :class="{ active: statusFilter === '' }" @click="setFilter('')">
        全部 <b>{{ devices.length }}</b>
      </button>
      <button
        class="chip"
        :class="{ active: statusFilter === '正常使用' }"
        @click="setFilter('正常使用')"
      >
        正常使用 <b>{{ activeCount }}</b>
      </button>
      <button
        class="chip"
        :class="{ active: statusFilter === '维修中' }"
        @click="setFilter('维修中')"
      >
        维修中 <b>{{ repairCount }}</b>
      </button>
      <button class="chip" :class="{ active: statusFilter === '闲置' }" @click="setFilter('闲置')">
        闲置 <b>{{ idleCount }}</b>
      </button>
      <button
        class="chip chip-scrapped"
        :class="{ active: statusFilter === '已报废' }"
        @click="setFilter('已报废')"
      >
        已报废 <b>{{ scrappedCount }}</b>
      </button>
    </section>
    ...
    <section class="toolbar">
      <input
        v-model="searchQuery"
        placeholder="搜索名称 / 型号 / 科室 / 编号"
        class="search-input"
      />
      <span class="result-count"
        >共 <b>{{ filteredDevices.length }}</b> 台</span
      >
      <div class="action-bar">
        ...
        >
          删除选中<span v-if="batchSelectedCount" class="cnt">{{ batchSelectedCount }}</span>
        </button>
        <button class="btn btn-clear" @click="confirmClearScrapped" :disabled="scrappedCount === 0">
          清空报废<span v-if="scrappedCount" class="cnt">{{ scrappedCount }}</span>
        </button>
    ...
    <section class="table-card" ref="tableWrap" @mousedown="startDrag">
      ...
      <table class="device-table" v-if="filteredDevices.length">
        ...
        <tbody>
          <tr
            ...
            class="draggable"
            :class="{
              scrapped: device.status === '已报废',
              sel: selectedIds.has(device.id),
              fail: device.runStatus === '故障',
            }"
          >
            <td class="col-check">
              <input
            ...
            <td>
              <span class="badge" :class="'st-' + stClass(device.status)">{{ device.status }}</span>
            </td>
            <td>
              <select
                class="run-sel"
                :class="'run-' + runClass(device.runStatus)"
                :value="device.runStatus"
                @change="changeRunStatus(device, $event.target.value)"
              >
      ...
      <div v-else class="empty">
        {{ searchQuery || statusFilter ? '未找到匹配设备' : '暂无设备数据' }}
      </div>
      <div
        v-if="dragRect"
        class="drag-box"
    ...
    <div v-if="showModal && !showBatchAdd" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-head">
          <h3>{{ editingDevice ? '编辑设备信息' : '新增医疗设备' }}</h3>
        ...
        <form class="modal-body" @submit.prevent="submitForm">
          ...
          <div class="row2">
            <label class="f"
              >所属科室 *<select v-model="form.department" required>
                <option value="">请选择</option>
                <option v-for="d in departmentOptions" :key="d" :value="d">{{ d }}</option>
              </select></label
            >
            <label class="f"
              >购置日期 *<input type="date" v-model="form.purchaseDate" required
            /></label>
          </div>
          <div class="row2">
            <label class="f"
            ...
            /></label>
            <label class="f"
              >使用状态<select v-model="form.status">
                <option v-for="s in ['正常使用', '维修中', '闲置', '已报废']" :key="s" :value="s">
                  {{ s }}
                </option>
              </select></label
            >
          ...
          <label class="f"
            >运行状态<select v-model="form.runStatus">
              <option v-for="r in runStatusOptions" :key="r" :value="r">{{ r }}</option>
            </select></label
          >
          <label class="f"
            >备注<textarea v-model="form.note" maxlength="200" rows="2"></textarea>
          </label>
          <div class="modal-foot">
            <button type="button" class="btn ghost" @click="closeModal">取消</button
            ><button type="submit" class="btn pri">
              {{ editingDevice ? '保存修改' : '确认录入' }}
            </button>
          </div>
        </form>
      </div>
    ...
    <div v-if="showBatchAdd" class="modal-overlay" @click.self="closeModal">
      <div class="modal modal-batch">
        <div class="modal-head">
          <h3>批量新增设备（{{ batchRows.length }} 行）</h3>
          <button class="x" @click="closeModal">✕</button>
        </div>
        <div class="modal-body batch-body">
          <div class="batch-table">
            <div class="brow bhead">
              <span>设备名称*</span><span>型号</span><span>所属科室*</span><span>购置日期*</span
              ><span>价格(元)</span><span>使用状态</span><span>备注</span>
            </div>
        ...
        <div class="modal-foot">
          <button type="button" class="btn ghost" @click="closeModal">取消</button
          ><button type="button" class="btn pri" @click="submitBatch">批量提交</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showDeleteConfirm" class="del-overlay" @click.self="cancelDelete">
      <div class="del-dialog">
        ...
        <p v-if="deleteTarget" class="del-text">
          确定要删除设备 <b class="del-name">{{ deleteTarget.name }}</b
          ><br />（{{ deleteTarget.code }}）<br /><span class="del-warn">此操作不可恢复！</span>
        </p>
        <div class="del-btns">
    ...
    <div v-if="showBatchConfirm" class="del-overlay" @click.self="showBatchConfirm = false">
      <div class="del-dialog">
        ...
        <p class="del-text">
          确定要批量删除选中的
          <b class="del-name">{{ batchSelectedCount }}</b> 台设备吗？<br /><span class="del-warn"
            >此操作不可恢复！</span
          >
        </p>
        <div class="del-btns">
...
.device-table th {
  ...
  background: #f2f7f2;
  color: #557457;
  border-bottom: 2px solid #b8d8b9;
  white-space: nowrap;
}
.device-table td {
  padding: 9px 12px;
...
.device-table tbody tr.scrapped {
  background: #fefafa;
  opacity: 0.7;
}
.device-table tbody tr.sel {
  background: #eef6ee;
}
.device-table tbody tr.fail {
  background: #fdecec;
}
.device-table tbody tr.fail:hover {
  background: #fbdfe0;
}
.drag-box {
  position: fixed;
  background: rgba(74, 133, 77, 0.18);
  border: 1px solid rgba(74, 133, 77, 0.6);
  pointer-events: none;
  z-index: 5;
}
.col-check {
...
.brow {
  display: grid;
  grid-template-columns: 1.2fr 1fr 1fr 1fr 0.9fr 1fr 1.2fr;
  gap: 6px;
  margin-bottom: 8px;
  align-items: center;
}
.brow:not(.bhead) {
  grid-template-columns: 1.2fr 1fr 1fr 1fr 0.9fr 1fr 1.2fr 28px;
}
.brow input,
.brow select {
  padding: 7px 8px;
