...
<template>
  <div class="schedule-page">
    ...
    <section class="toolbar">
      <label class="search-field">
        ...
        <div class="suggest-wrap">
          ...
          <div v-if="deptSuggest.length" class="suggest-list">
            <div v-for="d in deptSuggest" :key="d" class="suggest-item" @click="deptKeyword = d">
              {{ d }}
            </div>
          </div>
        </div>
      ...
      <label class="search-field">
        ...
        <div class="suggest-wrap">
          ...
          <div v-if="doctorSuggest.length" class="suggest-list">
            <div
              v-for="d in doctorSuggest"
              :key="d.id"
              class="suggest-item"
              @click="doctorKeyword = d.name"
            >
              {{ d.name }}
            </div>
          </div>
      ...
      <span class="count"
        >共 <b>{{ filtered.length }}</b> 条安排</span
      >
    </section>
      ...
      <table class="schedule-table">
        ...
        <tbody>
          <tr v-for="s in filtered" :key="s.id">
            ...
            <td>{{ s.timeRange }}</td>
            <td>
              <span class="status" :class="s.status === '出诊' ? 'out' : 'sched'">{{
                s.status
              }}</span>
            </td>
            <td>{{ s.location }}</td>
