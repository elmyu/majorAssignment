...
<template>
  <div class="dashboard-page">
    ...
    <section class="run-summary">
      <div
        class="run-item"
        :class="{ sum_active: runFilter === '在线' }"
        @click="runFilter = runFilter === '在线' ? '' : '在线'"
      >
        在线 <b>{{ runCount['在线'] }}</b>
      </div>
      <div
        class="run-item"
        :class="{ sum_active: runFilter === '运行中' }"
        @click="runFilter = runFilter === '运行中' ? '' : '运行中'"
      >
        运行中 <b>{{ runCount['运行中'] }}</b>
      </div>
      <div
        class="run-item"
        :class="{ sum_active: runFilter === '故障' }"
        @click="runFilter = runFilter === '故障' ? '' : '故障'"
      >
        故障 <b>{{ runCount['故障'] }}</b>
      </div>
      <div
        class="run-item"
        :class="{ sum_active: runFilter === '校准中' }"
        @click="runFilter = runFilter === '校准中' ? '' : '校准中'"
      >
        校准中 <b>{{ runCount['校准中'] }}</b>
      </div>
      <div
        class="run-item"
        :class="{ sum_active: runFilter === '离线' }"
        @click="runFilter = runFilter === '离线' ? '' : '离线'"
      >
        离线 <b>{{ runCount['离线'] }}</b>
      </div>
    </section>
