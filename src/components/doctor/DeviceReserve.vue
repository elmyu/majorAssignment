...
const filteredDevices = computed(() => {
  ...
  if (k) r = r.filter((d) => d.name.toLowerCase().includes(k) || (d.code || '').toLowerCase().includes(k))
  return r
})
...
function doReserve() {
  if (!reserveDeviceId.value) { showToast('请先选择要预约的设备', 'warning'); return }
  if (!reserveDate.value) { showToast('请选择预约日期', 'warning'); return }
  const timeStr = computedTimeRange.value
  if (!timeStr) { showToast('请设置预约时间段', 'warning'); return }
  try {
    createReservation({
...
<template>
  <div class="reserve-page">
    <div class="head-desc">设备预约：选择可用设备，设置预约日期与时间段，提交生成预约记录（同一时段不可重复预约）。</div>
    ...
    <section class="card form-card">
      ...
      <div class="reserve-layout">
        ...
        <div class="form-left">
          <div class="form-row">
            <label class="field full">
              ...
              <select v-model="reserveDeviceId" @change="selectDevice(reserveDeviceId)">
                ...
                <option v-for="d in bookable" :key="d.id" :value="d.id">{{ d.name }}（{{ d.code }} · {{ d.runStatus }}）</option>
              </select>
            </label>
          </div>
          <div class="form-row">
            ...
            <label class="field">
              ...
              <select v-model="reserveSlot">
                <option v-for="s in slotOptions" :key="s.value" :value="s.value">{{ s.value }}（{{ s.range || '手动填写' }}）</option>
              </select>
            </label>
          </div>
          ...
          <p v-if="isConflict" class="conflict-msg">⚠ 该设备在所选日期与时间段已有预约，请更换时间。</p>

          <button type="button" class="submit-btn" @click="doReserve" :disabled="isConflict">提交预约</button>
        </div>
        ...
        <div class="form-right">
          ...
          <div class="avail-grid">
            ...
            >
              ...
              <div class="avail-meta">{{ d.department }} · <span class="run-ok">{{ d.runStatus }}</span></div>
            </div>
...
.card { background: #fff; border: 1px solid #eef2ee; padding: 16px 18px; }
.card h3 { margin: 0 0 14px; font-size: 1rem; color: #1f2937; }

.reserve-layout { display: flex; gap: 20px; }
.form-left { flex: 1.3; display: flex; flex-direction: column; gap: 12px; }
.form-right { flex: 1; border-left: 1px solid #eef2ee; padding-left: 18px; }
.form-right h4 { margin: 0 0 10px; font-size: 0.92rem; color: #1f2937; }

.form-row { display: flex; gap: 12px; }
.field { display: flex; flex-direction: column; gap: 5px; flex: 1; }
.field.full { width: 100%; }
.field span { font-size: 0.82rem; font-weight: 600; color: #4b5563; }
.req { color: #c0392b; font-style: normal; }
.field select, .field input { padding: 9px 10px; border: 1px solid #e0e7e0; font-size: 0.85rem; font-family: inherit; background: #fff; }

.filter-row { display: flex; gap: 8px; margin-bottom: 12px; }
.filter-input { flex: 1; min-width: 0; padding: 8px 10px; border: 1px solid #e0e7e0; font-size: 0.85rem; font-family: inherit; }
.filter-select { padding: 8px 6px; border: 1px solid #e0e7e0; font-size: 0.85rem; font-family: inherit; background: #fff; }

.avail-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 8px; max-height: 200px; overflow-y: auto; }
.avail-item { display: flex; flex-direction: column; gap: 2px; padding: 8px 10px; border: 1px solid #eef2ee; cursor: pointer; background: #fbfdfb; }
.avail-item:hover { border-color: #9cc39f; background: #f0f8f0; }
.avail-item.picked { border-color: #4a854d; background: #e7f5e7; }
.avail-name { font-weight: 600; color: #1f2937; font-size: 0.85rem; }
.avail-meta { font-size: 0.72rem; color: #9ca3af; }
.avail-empty { grid-column: 1 / -1; text-align: center; padding: 24px; color: #b0b0b0; }
.run-ok { color: #2d7d4f; font-weight: 600; }

.confirm-bar { display: flex; justify-content: space-between; align-items: center; gap: 10px; flex-wrap: wrap; background: #f6faf6; border: 1px solid #dbe8db; padding: 9px 12px; font-size: 0.84rem; }
.confirm-info { display: flex; align-items: center; flex-wrap: wrap; gap: 6px; color: #2c6e33; }
.confirm-info .sep { color: #b9cfba; }
.confirm-info .code { font-family: monospace; color: #6b7280; font-size: 0.76rem; }
.confirm-info .hint { color: #9ca3af; }
.confirm-time { font-weight: 700; color: #2c6e33; }
.conflict-msg { color: #c0392b; font-size: 0.85rem; margin: 0; }
.submit-btn { padding: 11px 24px; border: none; color: #fff; font-weight: 700; font-size: 0.9rem; cursor: pointer; background: #4a854d; font-family: inherit; align-self: flex-start; }
.submit-btn:hover { background: #3d6b40; }
.submit-btn:disabled { background: #b7c9b7; cursor: not-allowed; }
...
.toast { position: fixed; top: 84px; right: 24px; padding: 10px 16px; color: #fff; font-size: 0.85rem; font-weight: 600; z-index: 3000; }
.toast-success { background: #2f9e5f; }
.toast-warning { background: #df8c1a; }
.toast-info { background: #3d7fc4; }
@media (max-width: 900px) { .reserve-layout { flex-direction: column; } .form-right { border-left: none; padding-left: 0; border-top: 1px solid #eef2ee; padding-top: 14px; } }
</style>
