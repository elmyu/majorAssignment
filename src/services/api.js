...
export function getSchedules() {
  ...
  const schedules = tables
    .schedules()
    .map((s) => {
      const doc = doctors.find((d) => d.id === s.doctorId);
      return {
        ...s,
        doctorName: doc ? doc.name : '未知',
        doctorDept: doc ? doc.dept : '',
        doctorTitle: doc ? doc.title : '',
      };
    })
...
export function getPatientsForDoctor() {
  assertRole(['doctor']);
  const users = tables.users();
  const patients = users.filter((x) => x.role === 'patient').map(({ password, ...safe }) => safe);
  return patients;
}
...
export function createDevice(data) {
  assertRole(['admin']);
  if (!data.name || !data.department || !data.purchaseDate)
    throw new BizError('请填写设备名称、科室与购置日期');
  const devices = tables.devices();
